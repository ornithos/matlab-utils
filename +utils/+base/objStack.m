classdef objStack < handle
% objStack define a stack data strcuture
% 
% It likes java.util.Stack, however, it could use objStack.content() to
% return all the data (in cells) of the Stack, and it is a little faster
% than java's Stack.
% 
%   s = objStack(c);  c is a cells, and could be omitted
...
% 
% 定义了一个栈
% s = objStack; 定义一个空的栈对象
% s = objStack(c); 定义栈对象，并用c初始化s，当c为cell时，c的元素为队列的数据，
%    否则c本身为队列的第一个数据
%
% 支持操作：
...
% url: http://zhiqiang.org/blog/it/matlab-data-structures.html
    ...
    methods
        ...
        function el = top(obj)
            if obj.cur == 0
                el = [];
                warning('objStack:No_Data', 'trying to get top element of an empty stack');
            else
        ...
        function el = pop(obj)
            if obj.cur == 0
                el = [];
                warning('objStack:No_Data', 'trying to pop element of an empty stack');
            else
                el = obj.buffer{obj.cur};
                obj.cur = obj.cur - 1;
            end        
        ...
        function c = contents(obj)
            c = obj.buffer(1:obj.cur);
        end
        
        function tf = ismember(obj, el)
            c = obj.contents;
            tf = ismember(el, c);
        end
        
        function uniquePush(obj, el)
            if ~obj.ismember(el)
                obj.push(el);
            end
        end
